...
const testimonials = [
  {
    ...
    name: "김0정",
    quote: "정책 소통에 AI와 빅데이터를 어떻게 활용할지 궁금했는데, 실무 적용 방향과 출처 검증 방법까지 함께 익힐 수 있었습니다.",
    followUp: "NotebookLM·Perplexity를 활용한 정책자료 비교·검증과 인포그래픽 제작을 더 깊이 배우고 싶습니다.",
  },
  {
  ...
  },
  {
    ...
    name: "한0진",
    quote: "코딩을 몰라도 공문 작성과 반복업무를 줄이는 도구를 직접 만들어 보니 업무혁신이 현실적으로 느껴졌습니다.",
    followUp: "후속 심화과정에서 팀 공용 자동화 도구를 완성하고 공유·배포하는 단계까지 배우고 싶습니다.",
  },
  {
  ...
  },
  {
    ...
    name: "서0우",
    quote: "데이터를 요약하는 데서 끝나지 않고 근거를 검증하고 시각화해 설명하는 과정까지 연결되어 실무 활용도가 높았습니다.",
    followUp: "데이터 품질 검증과 시각화 자동화를 실제 공공데이터 과제로 수행하는 심화교육에도 참여하고 싶습니다.",
  },
];
            ...
            <div className="testimonial-grid">
              {testimonials.map((item) => (
                <article className="testimonial-card" key={`${item.organization}-${item.name}`}>
                  ...
                  {item.followUp && <p className="follow-up-message"><strong>심화교육 희망</strong>{item.followUp}</p>}
                </article>
              ))}
            </div>
          </div>
